...
function Home() {
  ...
  /*const str = "Several generations following Caesar's reign, apes – now the dominant species – live harmoniously while humans have been reduced to living in the shadows. As a new tyrannical ape leader builds his empire, one young ape undertakes a harrowing journey that will cause him to question all he's known about the past and to make"

  console.log(str.length)*/

  //Import des films dans dataMovies via fetch

  useEffect(()=>{
    fetch('https://mymoviz-backend-gilt-six.vercel.app/movies')
      ...
      for (let element of data.movies)
        {let overviewDef = ''
          if (element.overview.length > 250){
            overviewDef=element.overview.slice(0,250) + '...'
          }
          else {overviewDef=element.overview}
          
          movieList.push({title : element.title, poster: `https://image.tmdb.org/t/p/original/${element.poster_path}`, voteAverage: element.vote_average , voteCount : element.vote_count, overview: overviewDef })}
